...
{
  uint8_t gpioxTag = platformGetGpioPort (ioRes);
  GPIO_TypeDef *gpiox = (GPIO_TypeDef *)(__STM32_GPIOX (gpioxTag));
  uint16_t pin = __STM32_PIN (platformGetGpioPin (ioRes));

  GPIO_InitTypeDef GPIO_InitStruct = { 0 };

  platformRccPeriph_t rccPeriph = ioGpioxRcc[gpioxTag].rcc;
  stm32RccClockCmd (rccPeriph, ENABLE);

  GPIO_InitStruct.Pin = pin;

  uint8_t major = platformGetMajorRes (ioRes);

  switch (major)
    {
    case PLATFORM_RES_MAJOR (M_OutputIO):
